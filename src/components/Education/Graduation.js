...
import { createEducationGraduation } from '../../redux/actions/education.js'
...
const Graduation = () => {
    ...
    const { error, message,loading } = useSelector(state => state.educationGraduation);
    ...
    const submitHandler = async (e) => {
        ...
        await dispatch(createEducationGraduation(myForm));
    ...
    useEffect(() => {
        ...
        if (message) {
            ...
            let activeIndex = 2;
            let activeIndexIndex = 'postgraduation';
    ...
    return (
            <Container display={'flex'} alignItems={'center'} minH={'70vh'} maxW={'100%'} pb={'6rem'}>
                ...
                <Button isLoading={loading} onClick={submitHandler} rightIcon={<FaArrowDown />} style={{ backgroundColor: '#85325c', color: 'white', }} pos={'absolute'} bottom={'13rem'} right={'20rem'} w={'9rem'} type='submit' mt={8}>
                    Save & Next
                </Button>
            </Container>
    )
